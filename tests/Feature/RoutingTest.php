...
test('root index view can be matched', function () {
    ...
    expect(realpath(__DIR__.'/../tmp/views/index.blade.php'))->toEqual($folio->match(new Request, '/')->path)
        ->and($folio->match(new Request, '/missing-view'))->toBeNull();
});
...
test('directory index views can be matched', function () {
    ...
    expect(realpath(__DIR__.'/../tmp/views/users/index.blade.php'))->toEqual($folio->match(new Request, '/users')->path);
});
...
test('literal views can be matched', function () {
    ...
    expect(realpath(__DIR__.'/../tmp/views/profile.blade.php'))->toEqual($folio->match(new Request, '/profile')->path);
});
...
test('wildcard views can be matched', function () {
    ...
    $resolved = $folio->match(new Request, '/1');
...
test('literal views may be in directories', function () {
    ...
    expect(realpath(__DIR__.'/../tmp/views/users/profile.blade.php'))->toEqual($folio->match(new Request, '/users/profile')->path);
});
...
test('wildcard views may be in directories', function () {
    ...
    $resolved = $folio->match(new Request, '/users/1');
...
test('wildcard views must be blade files', function () {
    ...
    expect($folio->match(new Request, '/users/1'))->toBeNull();
});
...
test('multisegment wildcard views can be matched', function () {
    ...
    $resolved = $folio->match(new Request, '/1/2/3');
...
test('wildcard directories are properly handled', function () {
    ...
    $resolved = $folio->match(new Request, '/flights/1/connections');
...
test('nested wildcard directories are properly handled', function () {
    ...
    $resolved = $folio->match(new Request, '/flights/1/connections/2/map');
